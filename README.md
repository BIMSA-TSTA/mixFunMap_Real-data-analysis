...
├── 04_rice_diversity.R        # Rice Diversity Panel 1 two-environment analysis (Figure 4)
...
## Wheat significance thresholds

Both wheat analyses use the primary LD-pruned threshold stored in
`results/wheat_height/thresholds.rds`: `0.05 / 3270` (approximately
`1.529e-5`, LD pruning at r2 = 0.2). Analysis summaries and figures use
this same threshold. The suggestive threshold remains `1 / 18583`.
The frozen mixFunMap summaries therefore report 3 significant height
markers and 12 significant canopy-cover markers. These counts were
updated from the existing scan P values; no model fitting or marker
scanning was repeated.
